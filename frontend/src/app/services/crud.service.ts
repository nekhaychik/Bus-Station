...
export class CrudService {
  ...
  public getDataDoc<T>(
    ...
    const snapshot: Observable<firebase.firestore.DocumentSnapshot<T>> =
  ...
  public createObject<T>(
    ...
    return (
      from(
        this.angularFirestore.collection(collectionName).add(object)
      ) as Observable<DocumentReference<T>>
    ).pipe(take(1));
  }
  ...
  public getOneData(collectionName: string, id: string) {
    return from(
      this.angularFirestore.collection(collectionName).doc(id).get()
    ).pipe(take(1));
  }
